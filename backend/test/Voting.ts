...
describe('Voting contract', function () {
	...
	describe('getOneProposal', function () {
		beforeEach(async () => {
			({ owner, voter1, voter2, voter3, voting } =
		...
		it('Should get an existing proposal', async function () {
			...
			expect(
				(await voting.connect(voter1).getOneProposal(1)).description,
			).to.equal('DEFAULT_PROPOSAL');
		});
		...
		it('Should fail trying to get a proposal without being a voter', async function () {
			await expect(voting.connect(voter1).getOneProposal(1)).to.be
	...
	describe('getVoter(address voterAddress)', function () {
		it('Should get existing voters', async function () {
			...
			expect(
				(await voting.connect(voter1).getVoter(voter1.address))
					.isRegistered,
			).to.equal(true);
			expect(
				(await voting.connect(voter1).getVoter(voter2.address))
					.isRegistered,
			).to.equal(false);
			expect(
				(await voting.connect(voter1).getVoter(voter3.address))
					.isRegistered,
			).to.equal(true);
		});
	...
	describe('addVoter(address voterAddress)', function () {
		...
		it('Should add a voter', async function () {
			...
			expect(
				(await voting.getVoter(owner.address)).isRegistered,
			).to.equal(true);
		});
	...
	describe('addProposal(string proposalDescription)', function () {
		...
		it('Should fail trying to add a proposal in the wrong workflow status', async function () {
			await voting.endProposalsRegistering();
			await expect(
				voting.connect(voter1).addProposal('New proposal'),
		...
		it('Should fail trying to add a proposal without being voter', async function () {
			await expect(voting.addProposal('New proposal')).to.be.revertedWith(
				"You're not a voter",
			);
		});
	...
	describe('setVote(uint proposalId)', function () {
		...
		it('Should fail trying to vote for a non existing proposal', async function () {
			await expect(
				voting.connect(voter1).setVote(42424242),
		...
		it('Should increment the proposal vote count', async function () {
			await voting.connect(voter1).setVote(1);
			expect(
				(await voting.connect(voter1).getOneProposal(1)).voteCount,
			).to.equal(1);
		});
	});
	...
	describe('startProposalRegistering()', function () {
		...
		it('Should register a genesis proposal for voters once proposals start', async function () {
			...
			expect((await voting.getOneProposal(0)).description).to.equal(
				'GENESIS',
			);
		});
	});
	...
	describe('fuzzing proposals and votes', function () {
			...
			for (const seed of seeds) {
				...
				expect(
					(
						await voting
							.connect(voter1)
							.getOneProposal(currentProposalId)
					).description,
				).to.equal(proposalDesc);
				currentProposalId++;
			}
		});
		...
		it('Should handle fuzzed voting loads with deterministic randomness', async function () {
			...
			for (const seed of proposalSeeds) {
				const proposalDesc = pseudoProposalString(
					seed,
					deterministicRandom(`${seed}-len`, 56) + 1,
				);
				await voting.connect(voter1).addProposal(proposalDesc);
				proposalId++;
			}
			...
			for (let index = 0; index < voters.length; index++) {
				const choice = deterministicRandom(
					`fuzz-vote-${index}`,
					totalProposals,
				);
